...
<%
    try
    {
        ...
        pst=con.prepareStatement("select * from vegetables order by vname");
        rs=pst.executeQuery();
    }
    catch(Exception e)
    {
		...
		<div class="top-nav">
			<span class="menu"><img src="images/menu.png" alt=""> </span>
				<ul>
					...
					<li class="active"><a href="ProfileVegitabels.jsp">VEGETABLES</a></li>
...
<div class="vegitables">
    <%int i=1;
    while(rs.next()){%>
    <form action="selectedvegetables.jsp" method="post">
        <div class="vimag" style="padding:10px 300px 0px 50px;">
            <img src="images/V-<%out.print(i);%>.jpg" width="180px" height="150px" id="imgid" >
        </div>
        <div clas="vinfo" style="padding-left: 300px;">
            <p style="color:white; font-weight: bold; font-size: 22px;"><%=rs.getString(1)%></p>
            <p style="color:white; font-weight: bold; font-size: 18px;"><%=rs.getString(2)%> (Hindi)</p>
            <p style="color:white; font-weight: bold; font-size: 18px;"><%=rs.getInt(3)%> (Rs) per kg</p>
            <p style="color:white; font-weight: bold; font-size: 18px;"><%=rs.getString(4)%></p>
        </div><input type="checkbox" name="vegetablename" value="<%=rs.getString(1)%>" style="
      width: 28px;
      height: 28px;
      background: #fcfff4;
