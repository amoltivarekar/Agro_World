...
<%
    try
    {
        ...
        pst=con.prepareStatement("select * from tools order by tname");
        rs=pst.executeQuery();
    }
    catch(Exception e)
    {
		...
		<div class="top-nav">
			<span class="menu"><img src="images/menu.png" alt=""> </span>
				<ul>
					...
					<li class="active"><a href="ProfileTools.jsp">TOOLS</a></li>
...
<div class="vegitables">
    <%int i=1;
    while(rs.next()){%>
    <form action="SelectedTools.jsp" method="post">
        <div class="vimag" style="padding:10px 300px 0px 50px;">
            <img src="images/T-<%out.print(i);%>.jpg" width="180px" height="150px" id="imgid" >
        </div>
        <div clas="vinfo" style="padding-left: 300px;">
            <p style="color:white; font-weight: bold; font-size: 22px;"><%=rs.getString(1)%></p>
            <p style="color:white; font-weight: bold; font-size: 18px;"><%=rs.getInt(2)%> .Rs</p>
            <p style="color:white; font-weight: bold; font-size: 18px;"><%=rs.getString(3)%></p>
        </div><input type="checkbox" name="toolname" value="<%=rs.getString(1)%>" style="
      width: 28px;
      height: 28px;
      background: #fcfff4;
