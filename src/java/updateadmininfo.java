...
public class updateadmininfo extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet updateadmininfo</title>");            
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet updateadmininfo at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try{
            ...
            PreparedStatement pst = con.prepareStatement("update agroadmin set adminname=?,emailid=?,address=?,mobno=?,username=?,password=? where adminname='"+sur+"'");
            pst.setString(1,sn);
            pst.setString(2,se);
            pst.setString(3,sa);
            pst.setString(4,sp);
